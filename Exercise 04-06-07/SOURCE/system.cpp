...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    acceptance = 1.0/(1.0 + exp(-_beta*delta_E));
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_PI*_rho*(1.0/(9.0*pow(_r_cut,9)) - 1.0/(3.0*pow(_r_cut,3)));
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk("../OUTPUT/kinetic_energy.dat");
        coutk << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutk.close();
        _nprop++;
        ...
        coutt << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutte << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        ...
        coutpr << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_pressure = true;
        _index_pressure = index_property;
        index_property++;
        _ptail = 32*M_PI*_rho*(1.0/(9.0*pow(_r_cut,9)) - 1.0/(6.0*pow(_r_cut,3))) * 3.0 * double(_npart);
        ...
        ofstream coutpr("../OUTPUT/magnetization.dat",ios::app);
        coutpr << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        ofstream coutpr("../OUTPUT/specific_heat.dat",ios::app);
        coutpr << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        ofstream coutpr("../OUTPUT/susceptibility.dat",ios::app);
        coutpr << "#         BLOCK:      ACTUAL_PE:         PE_AVE:          ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open() and _sim_type < 2){
    string comment;
    string particle;
    double x, y, z;
    int ncoord;
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure)  virial += 48.0 * (1.0/pow(dr,12) - 0.5/pow(dr,6)); // PRESSURE
        }
      }
    }
  }
  ...
  if (_measure_pressure){
      pressure_temp = _rho * _measurement(_index_temp) + (virial + _ptail) / (3.0*_volume);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv) _measurement(_index_cv) = pow(tenergy_temp*double(_npart),2);
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
...
void System :: get_temperature(){
  ofstream coutf;
  coutf.open("../OUTPUT/equilibration_temp.dat",ios::app);
  coutf << _measurement(_index_temp) << endl;
  coutf.close();
  return;
}

void System :: get_potential_energy(){
  ofstream coutf;
  coutf.open("../OUTPUT/instantaneous_penergy.dat",ios::app);
  coutf << _measurement(_index_penergy) << endl;
  coutf.close();
  return;
}
...
void System :: averages(int blk){
  ...
  if(_measure_cv) _average(_index_cv) = pow(_beta,2) * (_average(_index_cv) - pow(_average(_index_tenergy)*double(_npart),2)) / double(_npart); // specific heat
  if(_measure_gofr){
    for(int bin=0; bin<_n_bins; bin++){ // gofr normalization
      _average(_index_gofr + bin) /= (_rho * double(_npart) * 4.0*M_PI/3.0 * (pow(bin * _bin_size + _bin_size,3) - pow(bin * _bin_size,3)));
    }
  }
  _global_av  += _average;
  ...
  if (_measure_penergy){
    ...
    coutf << setw(16) << blk
          << setw(16) << scientific << average
          << setw(16) << scientific << sum_average/double(blk)
          << setw(16) << scientific << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    if(blk == _nblocks){ // last block only
      for(int bin=0; bin<_n_bins; bin++){
        coutf.open("../OUTPUT/gofr.dat",ios::app);
        sum_average = _global_av(_index_gofr + bin);
        sum_ave2 = _global_av2(_index_gofr + bin);
        coutf << setw(16) << bin * _bin_size
              << setw(16) << scientific << sum_average/double(blk)
              << setw(16) << scientific << this->error(sum_average, sum_ave2, blk) << endl;
        coutf.close();
      }
  ...
  coutf << setw(12) << blk << setw(16) << fraction << endl;
  coutf.close();
